...
@interface PERRecordViewLayout ()

@property (nonatomic, strong) NSMutableArray *attributesArray; //总的布局对象数组，包括item，sectionHeader，footerHeader
@property (nonatomic, strong) NSMutableArray *itemsattributes; //item的布局对象数组
@property (nonatomic, strong) NSMutableArray *headerAttributes; //header的布局对象数组
@property (nonatomic, strong) NSMutableArray *footerAttributes; //footer的布局对象数组
@property (nonatomic, assign) CGFloat contentHeight; // 计算 collectionview 的内容高度
@property (nonatomic, assign) CGFloat viewWidth; //collectionview 自身的宽度

@end

@implementation PERRecordViewLayout
...
- (void)setItemFrameWithIndexPath:(NSIndexPath *)indexPath {
    ...
    if ( lastAttributes ) {
        ...
        if ( lastAttributes.indexPath.section == indexPath.section ) {
            //同一组
            if ( CGRectGetMaxX(lastAttributes.frame) + self.space + width > self.viewWidth ) {
                //需要换行
                x = self.itemInset.left + self.lineSpacing;
                y = CGRectGetMaxY(lastAttributes.frame) + self.lineSpacing;
            }else {
                //不需要换行
                x = CGRectGetMaxX(lastAttributes.frame) + self.space;
            ...
            x = self.itemInset.left + self.lineSpacing;
            y = CGRectGetMaxY(lastAttributes.frame) + self.lineSpacing * 2 + self.headerHeight;
        }
    }else {
